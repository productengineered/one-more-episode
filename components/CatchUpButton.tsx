...
export function CatchUpButton({ episodeId }: { episodeId: number }) {
  ...
  return (
    <button
      ...
      className="h-7 rounded-full border border-zinc-700 bg-zinc-800/60 px-2 text-xs text-zinc-400 transition-colors hover:border-zinc-500 hover:text-zinc-200 disabled:opacity-50"
    >
      {pending ? "…" : "⇤ here"}
    </button>
  );
